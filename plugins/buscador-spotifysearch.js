...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    if (!text) throw `${lenguajeGB['smsAvisoMG']()}𝙄𝙉𝙎𝙀𝙍𝙄𝙎𝘾𝙄 𝙄𝙇 𝙉𝙊𝙈𝙀 𝘿𝙄 𝙐𝙉 𝘼𝙍𝙏𝙄𝙎𝙏𝘼 𝘿𝙄 𝙎𝙋𝙊𝙏𝙄𝙁𝙔\n𝙀𝙎𝙀𝙈𝙋𝙄𝙊:\n${usedPrefix + command} tini`
    try {
    let risultati = await spotifyxv(text)
    let res = risultati.map((v, i) => {
    let durata = timestamp(v.duracion)
    return `[${i + 1}]\n❤️꙰༻ *TITOLO:* ${v.nombre}\n⁖👤༻ *ARTISTI:* ${v.artistas.join(', ')}\n⁖🗂️༻ *ALBUM:* ${v.album}\n⁖⏰༻ *DURATA:* ${durata}\n📎꙰༻ *LINK:* ${v.url}\n\n••••••••••••••••••••••••••••••••••••\n`    
}).join('\n')
if (res) {
