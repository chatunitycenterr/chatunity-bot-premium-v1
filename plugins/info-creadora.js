//CÓDIGO CREADO GRACIAS A https://github.com/AzamiJs & https://github.com/ChatUnity
...
let pp = gataImg
const cat = `ChatUnity-bot-premium-v1 💖💬
* ${bot}

*---------------------*

*CENTER Chatunity-bot*
*chatunitycenter@gmail.com*
...
description = official[0][0] == String(contact[0]) ? 'Solo temas de Chatunity-bot' : official[1][0] == String(contact[0]) ? lenguajeGB.smsContacto4() : official[2][0] == String(contact[0]) ? lenguajeGB.smsContacto4() : official[3][0] == String(contact[0]) ? lenguajeGB.smsContacto4() : desc === '' ? lenguajeGB.smsContacto5() : desc
...
enlace = official[0][0] == String(contact[0]) ? 'https://github.com/ChatUnity' : official[1][0] == String(contact[0]) ? 'https://github.com/elrebelde21' : official[2][0] == String(contact[0]) ? 'https://github.com/AzamiJs' : official[3][0] == String(contact[0]) ? 'https://github.com/Abiguelreyes75' : md 
...
lista.push([conn.user.jid.split('@')[0], await conn.getName(conn.user.jid), packname, lenguajeGB.smsContacto8(), mail === '' ? 'chatunitycenter@gmail.com' : mail, lenguajeGB.smsContacto7(), md, bioBot, yt, ig, fb, paypal, nna])
await conn.sendFile(m.chat, pp, 'lp.jpg', cat, fkontak, false, { contextInfo: {externalAdReply :{ mediaUrl: null, mediaType: 1, description: null, title: gt, body: ' 💬 𝗦𝘂𝗽𝗲𝗿 chatunity-bot-premium-v1 - 𝗪𝗵𝗮𝘁𝘀𝗔𝗽𝗽 ', previewType: 0, thumbnail: gataImg, sourceUrl: accountsgb.getRandom()}}})
...
//------- Nombre
  ...
  let insta = `https://www.instagram.com/chatunity_`
  let teksnomor = `
• @${wm.split`@`[0]} •
------- ${wm} -------
`
...
let teksbio = `ChatUnity-bot-premium-v1 💖💬
*Wa.me/573136855110*
𝙂𝙖𝙩𝙖𝘽𝙤𝙩𝙇𝙞𝙩𝙚-𝙈𝘿 💖💬
*wa.me/593993684821*
*---------------------*
    ...
    if (/(contacto|owner|creator|propietario|dueño|dueña|propietaria|dueño|creadora|creador)/i.test(command)) {
      const count = args[1] && args[1].length > 0 ? Math.min(99999999, Math.max(parseInt(args[1]), 1)) : !args[1] || args.length < 3 ? 1 : Math.min(1, count)
        switch (type) {
         ...
         // conn.reply(m.chat, "Nombre del bot : chatunity-bot-premium-v1 💬", m, { contextInfo: { mentionedJid: [nowner] }})
            break
            case 'bio':
