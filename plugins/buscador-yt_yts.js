...
if (!text) return conn.reply(m.chat, `${lenguajeGB['smsAvisoMG']()}𝙄𝙉𝙎𝙀𝙍𝙄𝙎𝘾𝙄 𝙄𝙇 𝙉𝙊𝙈𝙀 𝘿𝙄 𝙐𝙉 𝙑𝙄𝘿𝙀𝙊 𝙊 𝙐𝙉 𝘾𝘼𝙉𝘼𝙇𝙀 𝘿𝙄 𝙔𝙊𝙐𝙏𝙐𝘽𝙀\n\n𝙒𝙍𝙄𝙏𝙀 𝙏𝙃𝙀 𝙉𝘼𝙈𝙀 𝙊𝙁 𝘼 𝙔𝙊𝙐𝙏𝙐𝘽𝙀 𝙑𝙄𝘿𝙀𝙊 𝙊𝙍 𝘾𝙃𝘼𝙉𝙉𝙀𝙇`, fkontak,  m)
m.react('📀');
let result = await yts(text);
let ytres = result.videos;
if (!ytres.length) return m.reply('❌ Nessun risultato trovato.');

if (m.isWABusiness) {
let textoo = `${htki} 𝙍𝙄𝙎𝙐𝙇𝙏𝘼𝙏𝙄 ${htka}  ${text}\n\n`;
for (let i = 0; i < Math.min(15, ytres.length); i++) { 
let v = ytres[i];
textoo += `❤️꙰༻ *TITOLO:* ${v.title}\n⁖📆༻ *PUBBLICATO DA:* ${v.ago}\n⁖👀༻ *VISUALIZZAZIONI:* ${v.views}\n⁖⏰༻ *DURATA:* ${v.timestamp}\n📎꙰༻ *LINK:* ${v.url}\n\n••••••••••••••••••••••••••••••••••••\n\n`;
}
...
v.image, 
[],
[["Copia per scaricare", `.ytmp4 ${v.url}`]], 
[], 
[]]);

await conn.sendCarousel(m.chat, `${htki} *𝙍𝙄𝙎𝙐𝙇𝙏𝘼𝙏𝙄* ${htka}\n`, htki + "YouTube Search" + htka, messages, m);
